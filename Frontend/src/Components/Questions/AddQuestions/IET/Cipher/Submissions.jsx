...
const AdminSubmissionsDashboard = () => {
  ...
  const handleStatusChange = (submissionId, field, value) => {
    // Track changes in the modifiedSubmissions state without updating original data
  ...
  // Check if a submission has been modified
  const isModified = (submissionId) => {
    return !!modifiedSubmissions[submissionId];
  };

  // Get the current value for a field (either from modifications or original)
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      // Clear tracking of modifications
