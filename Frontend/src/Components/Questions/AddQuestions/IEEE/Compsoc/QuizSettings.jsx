...
const QuizSettings = ({ totalMarks, totalQuestions }) => {
  ...
  const fetchQuizSettings = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/questions/ieee/compsoc/settings', {
        method: 'GET',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/questions/ieee/compsoc/settings/update", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem('token')}`
