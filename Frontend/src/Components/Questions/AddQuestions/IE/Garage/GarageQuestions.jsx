...
const AdminQuestions = () => {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/questions/ie/garage/get', {
        method: 'GET',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const settingsResponse = await fetch('http://localhost:8080/questions/ie/garage/settings', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      const response = await fetch("http://localhost:8080/questions/ie/garage/add", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem('token')}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this question?")) {
      try {
        const response = await fetch(`http://localhost:8080/questions/ie/garage/delete/${id}`, {
          method: "DELETE",
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const updateMarks = async () => {
    try {
      const response = await fetch(`http://localhost:8080/questions/ie/garage/updateMarks/${editMarks.id}`, {
        method: "PATCH",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
