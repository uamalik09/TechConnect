...
const AdminSubmissionsDashboard = () => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8080/results/ie/script/submissions', {
          method: 'GET',
          headers: {
  ...
  const handleInterviewSlotChange = (submissionId, value) => {
    ...
    if (value) {
      // script a date object that correctly interprets the local datetime
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      const updatePromises = Object.entries(modifiedSubmissions).map(([submissionId, changes]) => {
        return fetch(`http://localhost:8080/results/ie/script/submissions/${submissionId}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
