...
const TorsionChat = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const res = await fetch("http://localhost:8080/api/doubts/1/3", {
        method: "GET",
        headers: {
  ...
  const handleAdminReply = async (doubtId) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8080/api/doubts/1/3/admin/${doubtId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <h1 className="text-2xl font-bold mb-4">Torsion Doubts</h1>
