...
const SigChat = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const res = await fetch(`https://tech--connect.azurewebsites.net/api/doubts/3/${sigId}`, {
        method: "GET",
        headers: {
  ...
  const handleAdminReply = async (doubtId) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://tech--connect.azurewebsites.net/api/doubts/3/${sigId}/admin/${doubtId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
