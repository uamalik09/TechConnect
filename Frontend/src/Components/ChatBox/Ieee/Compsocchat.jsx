...
const CompsocChat = () => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch('/api/doubts/2/1');
  ...
  const handleAdminReply = async (doubtId) => {
    try {
      const res = await fetch('/api/doubts/2/1/admin', {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <h1 className="text-2xl font-bold mb-4">Compsoc Doubts</h1>
